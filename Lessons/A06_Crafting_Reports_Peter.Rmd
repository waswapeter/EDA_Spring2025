...
```{r task3.1, echo=F, fig.align='left', fig.cap="2018 monthly Ozone levels", out.height="40%"}
epa_data %>% 
  filter(Year == '2018') %>% 
  drop_na(Ozone) %>% 
  ggplot(aes(
    x=factor(Month,labels=month.abb),
    y=Ozone)
    ) + 
  geom_boxplot() +
  labs(
...
```{r task3.2,echo=F, fig.align='left', fig.cap="2019 monthly Ozone levels", out.height="40%"}
epa_data %>% 
  filter(Year == '2019') %>% 
  drop_na(Ozone) %>% 
  ggplot(aes(
